...
To change the delivery email: update the `mailto:` address in the `mailtoUrl` string in `js/main.js` — no external service or dashboard needed.
...
- **After every code change, check whether `AGENTS.md`, `README.md`, and `.github/copilot-instructions.md` need to be updated** to reflect the change. If any of those files contain information that is now out of date, update them in the same commit.
